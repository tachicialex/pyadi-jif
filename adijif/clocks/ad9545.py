...
class ad9545(clock):
    ...
    def _setup_solver_constraints(self, input_refs: List[int], out_freqs: List[int]) -> None:
        ...
        if self.solver == "gekko":
            ...
            """ Add divider as variables to the model """
            for i in range(0,4):
                self.config["r" + str(i)] = exp.integer_var(domain=(self.R_min, self.R_max))
                self.config["input_ref_" + str(i)] = exp.CpoValue(int(input_refs[i]), type=ctg.Type_Int)

            """ Add PLL N as variables to the model for each PLL profile """
            for i in range(0,2):
                if self.PLL_used[i]:
                    self.config["PLL" + str(i) + "_rate"] = exp.integer_var(domain=(int(self.PLL_out_min[i]), int(self.PLL_out_max[i])))

                for j in range(0,4):
                    if input_refs[j] != 0:
                        n_name = "n" + str(i) + "_profile_" + str(j)
                        self.config[n_name] = exp.integer_var(domain=(self.N_min, self.N_max))

                        """ Internally the PLL block is composed of a Digital PLL and an Analog PLL
                            with different constraints on dividers.
                        """
                        DPLL_N = exp.integer_var(domain=(1, 300000000))
                        APLL_M = exp.integer_var(domain=(1, 255))
        ...
        for i in range(0, 4):
            if self.input_refs[i] != 0:
                if self.solver == "gekko":
                    ...
                    self.config["PLL_in_rate_" + str(i)] = exp.integer_var(domain=(self.PLL_in_min, self.PLL_in_max))
    ...
    def set_requested_clocks( self, input_refs: List, out_freqs: List) -> None:
        """
        ...
        for i in range(0,10):
            if out_freqs[i] != 0:
                if self.solver == "gekko":
                    ...
                    self.config["q" + str(i)] = exp.integer_var(domain=(self.Q_min, self.Q_max))
